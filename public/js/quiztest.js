...
const terminateQuiz = async () => {
  try {
    const resp = await fetch('/admin/terminate/1', {
      method: 'GET',
      headers: {
    ...
    if (resp.status === 200) {
      ...
      let message = jsonResp.terminateById.message;
      let counter = jsonResp.terminateById.counter;

      const markup = `<div class="terminateQuiz">${message}<span id="counterValue"> ${counter}</span></div> `;
      document.querySelector('body').innerHTML = markup;

      const counterVal = document.querySelector('#counterValue');
      if (counterVal) {
        let value = counterVal.textContent.trim();
        const updCounter = updateCounter(value);
        setInterval(() => updCounter(counterVal), 1000);
      }
    }
  } catch (err) {
    alert('error', "Can't Update, try again later!!");
  }
};
const checkAllergie = (() => {
  let prevVal = '';
  const valuesToCheck = ['Banana', 'Olive', 'Sunflowers'];
  return async function (e) {
    e.preventDefault();
    let valueThatGeneratedClick = e.target.dataset.val;

    if (valuesToCheck.includes(prevVal.toString())) {
      await terminateQuiz();
    }
    prevVal = valueThatGeneratedClick;
  };
})();

$(document).on('click', '.noneSelected', handleNoneOfTheAbove);
  ...
  if (type == 1) {
    ...
    ques.answers.forEach((val, index) => {
      if (val.answer) {
        ...
        `);
        if (ques.answers.length - 1 === index) {
          $('#typeSelection .answerInner').append(`
            <button  class="selectionBtns noneSelected">None of the Above</button>
        `);
        }
      }
    });
    ...
    let selectionBtnsArr = Array.from(
      document.querySelectorAll('.selectionBtn')
    );
    console.log('selectionBtnsArr', selectionBtnsArr);
    for (let i = 0; i < selectionBtnsArr.length; i++) {
      selectionBtnsArr[i].addEventListener('click', checkAllergie);
    }
  } //type 7 ends here
  else if (type == 8) {
...
const terminatesForm = document.forms.terminateForm;

const updateTerminates = async (data, id) => {
  const url = '/admin/terminate/' + id;
  console.log(id);
  ...
  try {
    ...
    if (resp.status === 200) {
      ...
      alert('success', 'Terminate values successfully updated');
...
if (terminatesForm)
  terminatesForm.addEventListener('submit', (e) => {
    e.preventDefault();
    const data = new FormData(terminatesForm);
    let id = document.querySelector('#id').value;

    updateTerminates(data, id);
  });
